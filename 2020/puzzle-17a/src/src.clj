...
(defn initial-state [initial-state-representation]
  (let [rows (string/split-lines initial-state-representation)
    ...
    {:active (->>
              ...
              (map #(when (= (get (get rows (last %)) (first %)) \#) (flatten (seq [% 0]))))
              (filter some?)
              (map vec)
              set)
     :min-x -1
...
(defn next-state [min-x max-x min-y max-y min-z max-z active]
  (set
   (->>
    (cartesian-product (range min-x max-x) (range min-y max-y) (range min-z max-z))
    (map
     #(if (contains? active %)
...
(defn -main []
  (let [initial-state-representation (slurp "input.txt")
    ...
    (loop [i 0
           min-x (:min-x initial-state)
           max-x (:max-x initial-state)
           min-y (:min-y initial-state)
           max-y (:max-y initial-state)
           min-z -1
           max-z 2
           active (:active initial-state)]
      (if (= i 6)
        (println (count active))
        (recur (+ i 1) (- min-x 1) (+ max-x 1) (- min-y 1) (+ max-y 1) (- min-z 1) (+ max-z 1)
               (next-state min-x max-x min-y max-y min-z max-z active))))))