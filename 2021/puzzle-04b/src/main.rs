...
fn main() {
    ...
    let mut grid_sum_unmarked_numbers = -1;
    let mut i = 0;
    let mut grids_with_bingo = 0;
    while grids_with_bingo != grids.len() {
        let current_draw_number = drawed_numbers[i].parse::<i32>().unwrap();
        let mut j = 0;
        while grids_with_bingo != grids.len() && j != grids.len() {
            grids[j].mark_if_present(&drawed_numbers[i]);
            if !grids[j].bingo && (grids[j].has_complete_row() || grids[j].has_complete_column()) {
                last_drawn_number = current_draw_number;
                grid_sum_unmarked_numbers = grids[j].sum_unmarked_numbers();
                grids[j].bingo = true;
                grids_with_bingo += 1;
            }
            j += 1;
    ...
    println!("{}", last_drawn_number * grid_sum_unmarked_numbers);
}
