...
fn main() {
    ...
    let beacons_to_canonical = |i, beacons: &[Point], scanner_position| {
        beacons
            .iter()
            .map(|beacon| {
                beacon_canonical_position(&i, scanner_position, beacon, &scanner_to_permutation)
            })
            .flatten()
            .collect::<Vec<Point>>()
    };
    let canonical_beacons = scanners_beacons
        .iter()
        .enumerate()
        .flat_map(|(i, scanner_beacons)| {
            beacons_to_canonical(i, scanner_beacons, scanner_to_position.get(&i).unwrap())
        })
        .collect::<Vec<Point>>();
...
fn scanners_overlap_heuristic(
    ...
    for (i, j) in positions(sa_points.len()) {
        let d1 = sa_points[i].manhattan_distance(sa_points[j]);
        for (a, b) in positions(sb_points.len()) {
            let d2 = sb_points[a].manhattan_distance(sb_points[b]);
            if d1 == d2 {
...
fn beacon_canonical_position(
    scanner_index: &usize,
    point1: &Point,
    point2: &Point,
    scanner_to_permutation: &HashMap<usize, (usize, Permutation)>,
) -> Option<Point> {
    if let Some((i, permutation)) = scanner_to_permutation.get(scanner_index) {
        let mut relative_position = point2.permutate(*permutation);
        let mut k = i;
        while let Some((n, perm)) = scanner_to_permutation.get(&k) {
            relative_position = relative_position.permutate(*perm);
            k = n;
        }
        relative_position = relative_position + *point1;
